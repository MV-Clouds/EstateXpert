...
import getPayerDetails from '@salesforce/apex/CreateInstallmentInvoiceController.getPayerDetails';
import sendInvoiceEmail from '@salesforce/apex/CreateInstallmentInvoiceController.sendInvoiceEmail';
import getVFPageContent from '@salesforce/apex/InvoiceInstallmentVFController.getVFPageContent';
import getVFPagePDF from '@salesforce/apex/InvoiceInstallmentVFController.getVFPagePDF';
import saveInvoiceAsFile from '@salesforce/apex/InvoiceInstallmentVFController.saveInvoiceAsFile';
import { CurrentPageReference } from 'lightning/navigation';
import { CloseActionScreenEvent } from 'lightning/actions';

export default class CreateInstallmentInvoice extends LightningElement {
    @api recordId; // Now refers to Payment_Schedule__c record
    @track spinner = true;
    ...
    connectedCallback() {
        console.log('Payment Schedule Record ID:', this.recordId);
    ...
    async fetchPayerDetails() {
        try {
            ...
            const payerDetails = await getPayerDetails({ paymentScheduleId: this.recordId });
    ...
    async fetchInvoiceContent() {
        try {
            this.invoiceContent = await getVFPageContent({ paymentScheduleId: this.recordId });
    ...
    async handleDownload() {
        this.spinner = true;
        try {
            const base64Pdf = await getVFPagePDF({ paymentScheduleId: this.recordId });
            ...
            this.invoiceId = await saveInvoiceAsFile({ paymentScheduleId: this.recordId, base64Pdf: base64Pdf });
            this.handleClose();
    ...
    async handleSendEmail() {
        ...
        try {
            ...
            const result = await sendInvoiceEmail({ paymentScheduleId: this.recordId, pdfBlob: base64Pdf });
