 ...
 */
    ...
    */
    applyFieldFormat(fieldValue, format) {
        try {
            ...
            errorDebugger('displayInquiry', 'applyFieldFormat', error, 'warn', 'Error applying field format');
            return fieldValue;
        }
    }
    ...
    */
    fetchFilterConfiguration() {
        this.isLoading = true;
        getObjectFields({ objectApiName: 'MVEX__Inquiry__c', featureName: 'Suggested_Inquiry_Filters' })
            .then((result) => {
  ...
  */
    ...
    * @description: this method is used to handle logical expression change with live validation.
    *   Validates:
    ...
    */
    validateLogicalExpression(expr, mappingLength) {
        ...
        // Replace parentheses with spaced versions so they split cleanly
        ...
        // Build a flat token type sequence for pattern checking
        // Type: 'num' | 'op' | 'open' | 'close'
    ...
    */
    saveCondition() {
        ...
        if (isFieldValid && isOperatorValid && isValueValid) {
            ...
            const currentField = this.inquiryFieldObject.MVEX__Field_Name__c;
            const currentOp = this.selectedConditionOperator;
            const currentValue = this.selectedListingField;
    ...
    */
    fetchInquiryConfiguration() {
        getObjectFields({ objectApiName: 'MVEX__Inquiry__c', featureName: 'Suggested_Inquiry_Fields' })
            .then(result => {
                if (result && result.metadataRecords && result.metadataRecords.length > 0) {
                    try {
