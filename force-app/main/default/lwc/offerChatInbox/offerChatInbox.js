...
export default class OfferChatInbox extends NavigationMixin(LightningElement) {
    ...
    @track isEditing = false; // Tracks edit mode
    @track editedOffer = {};
    ...
    getInitialData(){
        ...
        try {
            ...
            console.log('offerRecord:', this.offerRecord);
            ...
            .then(combinedData => {
                ...
                if(combinedData.record){
                    ...
                    const index1 = keys[1] || 'Field 1';
                    const index2 = keys[2] || 'Field 2';

                    this.contactDetails = [
                        { label: index1, value: this.recordData[index1] || 'N/A' },
                        { label: index2, value: this.recordData[index2] || 'N/A' }
                    ];
    ...
    handleEdit() {
        this.isEditing = true;
        // Initialize editedOffer with current values
        this.editedOffer = { ...this.offerRecord };
    }
    ...
    handleCancel() {
        ...
        this.editedOffer = {}; // Clear edited values
    }
    
    handleSave() {
        // Validate Offer Date and Expiration Date
        const offerDate = new Date(this.editedOffer.MVEX__Offer_Date__c);
        const expirationDate = new Date(this.editedOffer.MVEX__Offer_Expiration_Date__c);

        if (offerDate > expirationDate) {
            this.showToast('Error', 'Offer Date must be less than or equal to Expiration Date.', 'error');
            return;
        }

        // Validate Amount
        const amount = parseFloat(this.editedOffer.MVEX__Offer_Amount__c);
        if (isNaN(amount) || amount <= 0) {
            this.showToast('Error', 'Amount must be a positive number greater than zero.', 'error');
            return;
        }
        
        const offerData = {
            Id : this.editedOffer.Id,
            MVEX__Offer_Date__c: this.editedOffer.MVEX__Offer_Date__c,
            MVEX__Offer_Expiration_Date__c: this.editedOffer.MVEX__Offer_Expiration_Date__c,
            MVEX__Offer_Amount__c: this.editedOffer.MVEX__Offer_Amount__c,
        }
    ...
    backToOfferManager(){
        try{
            let componentDef = {
                ...
                componentDef: "MVEX:offerManager",
                 attributes: {
