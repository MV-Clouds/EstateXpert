...
export default class RecordConfigCmp extends LightningElement {
    ...
    * @description: save checklist records.
    * Date: 10/09/2024
    * Created By: Vyom Soni
    */
    saveChecklistRecords() {
        try {
            ...
            saveMetadata({ checklistData, totalPages, objectApiName: this.objectApiName, featureName: this.featureName })
                ...
                .catch(error => {
                    errorDebugger('RecordConfigCmp', 'saveChecklistRecords', error, 'warn', 'Error occurred while saving checklist');
                    this.toast('Error', 'Error while updating checklist', 'error');
                });
        } catch (error) {
            errorDebugger('RecordConfigCmp', 'saveChecklistRecords', error, 'warn', 'Error occurred while preparing checklist records');
        }
    }
    ...
    */
    selectOption1Logic(event){
        ...
        this.checklistItems[index].fieldName = selectedOptionValue;
        this.checklistItems[index].value = selectedOptionValue;
        this.checklistItems[index].label = label;
        this.checklistItems[index].fieldType = type;
        if (type == 'DATE') {
            this.checklistItems[index].isDisable = false;
            this.checklistItems[index].picklist = this.dateOptions;
            this.checklistItems[index].format = this.dateOptions[0].value;
        } else if (type == 'DATETIME') {
            this.checklistItems[index].isDisable = false;
            this.checklistItems[index].picklist = this.dateTimeOptions;
            this.checklistItems[index].format = this.dateTimeOptions[0].value;
        } else {
            this.checklistItems[index].isDisable = true;
            this.checklistItems[index].format = null;
        }
    }
    ...
    */
    selectOptionParentLogic(event){
        ...
        const type = event.currentTarget.dataset.type;
        const index = event.currentTarget.dataset.index;
        this.checklistItems[index].fieldName = this.checklistItems[index].relationshipName + '.' + selectedOptionValue;
        this.checklistItems[index].value = this.checklistItems[index].relationshipName + '.' + selectedOptionValue;
        this.checklistItems[index].label = label;
        this.checklistItems[index].fieldType = type;
        if (type == 'DATE') {
            this.checklistItems[index].isDisable = false;
